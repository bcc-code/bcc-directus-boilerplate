...
export default defineHook(({ filter }, { env, exceptions }) => {
	...
	function createOpenIdFrontendAuth(provider: string) {
		...
		return async (_accountability: Accountability, { req }: { req: Request }, { database }: { database: Knex }) => {		
			const payload = jwt.decode(req.token, { json: true });
			if (payload?.iss !== issuer) return null;

			return await jwt.verify(req.token, (header: any, callback: any) => {
				jwksClient.getSigningKey(header.kid, (_err, key: any) => {
					callback(null, key.getPublicKey());
				});
			}, { audience, issuer, algorithms }, async (err: any, payload: JwtPayload) => {
				if (err || ! payload) {
				...
				const identifier = payload[identifierKey ?? 'sub'] ? String(payload[identifierKey ?? 'sub']) : email;

				if (!identifier) {
					throw new InvalidCredentialsException();
				}

				const {personId, hasMembership} = payload['https://members.bcc.no/app_metadata'] ?? {};
